...
export class SessionComponent implements OnInit, OnDestroy {
  ...
  getSessions(setup: Setup) {
    this.backend
      .getSessions(this.dashboard.user, setup.collectionName, setup.from, setup.to, setup.filter)
      .subscribe(sessions => {
  ...
  getPageviews(session: SessionD) {
    this.backend
      .getPageviews(this.dashboard.user, this.setup.collectionName, session.key)
